import { initializeApp, cert, getApps } from 'firebase-admin/app';
import { getFirestore } from 'firebase-admin/firestore';
import dotenv from 'dotenv';
import path from 'path';
import fs from 'fs';
import { fileURLToPath } from 'url';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

dotenv.config({ path: path.join(__dirname, '../.env') });

const saPath = process.env.FIREBASE_SERVICE_ACCOUNT_PATH || 'backend/serviceAccountKey.json';
let resolvedPath = path.resolve(__dirname, '..', saPath);
if (!fs.existsSync(resolvedPath)) {
  resolvedPath = path.resolve(__dirname, '../backend', saPath);
}

const serviceAccount = JSON.parse(fs.readFileSync(resolvedPath, 'utf8'));

if (!getApps().length) {
  initializeApp({ credential: cert(serviceAccount) });
}

const firestoreDb = getFirestore();

async function main() {
  const tenantId = 'TEST_1';
  const tablesRef = firestoreDb.collection('tenants').doc(tenantId).collection('tables');
  const getTable = async (t) => (await tablesRef.doc(t).get()).data()?.data || [];

  const orders = await getTable('orders');
  const payments = await getTable('payments');

  const targetInvoices = ['INV-1043', 'INV-1048', 'INV-1049', 'INV-1050'];
  const targetOrders = orders.filter(o => targetInvoices.includes(o.invoice_number));

  console.log('--- TARGET ORDERS ---');
  targetOrders.forEach(o => {
    console.log(`Invoice: ${o.invoice_number} | ID: ${o.id} | Date: ${o.order_date} | Net: ${o.net_amount}`);
  });

  console.log('\n--- PAYMENTS MATCHING OR NEAR THESE ORDERS ---');
  payments.forEach(p => {
    if (p.shop_id === 's_1786604089214') {
      console.log(`Payment ID: ${p.id} | Date: ${p.payment_date} | Amount: ${p.collected_amount} | OrderID: ${p.order_id || 'UNASSIGNED'} | Mode: ${p.payment_mode}`);
    }
  });

  process.exit(0);
}

main().catch(console.error);
